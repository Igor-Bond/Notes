 ...
 * рисует настоящие экраны и собирает нотный стан, а DOM вне браузера взять
 * неоткуда. Поэтому CI открывает ту же самую страницу настоящим браузером и
 * читает итог, который она кладёт в globalThis.
 *
...
const АДРЕС = process.argv[2] || 'http://127.0.0.1:4180/tests/index.html';
